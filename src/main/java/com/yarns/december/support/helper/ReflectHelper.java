	...
	public static void copyProperties(Object dest, Object source){
		...
		for (Field f : source.getClass().getDeclaredFields()) {
			if(! Modifier.isFinal(f.getModifiers())){
				Object value = ReflectHelper.getFieldValue(source, f.getName());
				if(value != null && ReflectHelper.getDeclaredField(dest, f.getName()) != null){
					ReflectHelper.setFieldValue(dest, f.getName(), value);
				}
			}
		}
	}
